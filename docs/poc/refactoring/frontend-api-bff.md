...
BFFパターン導入により `lib/api.ts` が冗長になったため削除した。
...
**実施した作業:**
1. 各hooksで直接 `fetch('/api/*')` を呼び出すよう修正
   - `useWatchlist.ts` - ウォッチリストAPI
   - `useTrades.ts` - トレードAPI
   - `usePerformance.ts` - パフォーマンスAPI
   - `useScreener.ts` - スクリーナーAPI
   - `useStockData.ts` - 株価データAPI
   - `useMarketStatus.ts` - マーケット状態API
2. `app/page.tsx` のヘルスチェックをインライン関数に変更
3. `lib/api.ts` を削除

**削除理由:**
- Route Handlersで型安全性・エラーハンドリングは確保済み
- 中間層が不要になり、コードがシンプルになった
